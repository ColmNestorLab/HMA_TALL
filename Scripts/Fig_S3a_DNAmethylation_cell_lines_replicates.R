################################################################################
############# DNA METHYLATION OF UNTREATED CELL LINES: REPLICATES ##############
################################################################################
...
# plot % CpG methylation of cell lines (individual replicates)
# divided into global and methylation at CpG islands

# Figure: Extended Data Fig. 3a
...
plot_S3a <- ggarrange(
  ...
  ggplot(data = untreated_global_CpGme, aes(x = reorder(sample_id, CpGmethylation_global), y = CpGmethylation_global, color = cell_line)) +
    geom_point(size = 3) +
  ...
  # CGI methylation
  ggplot(data = untreated_global_CpGme, aes(x = reorder(sample_id, CGI), y = CGI, color = cell_line)) +
    geom_point(size = 3) +
...
# export plot
pdf("plots_for_publication/S3a_DNAmethylation_cell_lines_replicates.pdf")
plot(plot_S3a)
dev.off()