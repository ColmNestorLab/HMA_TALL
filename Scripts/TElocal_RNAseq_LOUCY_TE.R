...
# Title: TElocal_RNAseq_LOUCY_TE_240805
...
set.seed(205) # seed for reproducibility
...
files <- list(i2_L_D10_3d_totalRNA_S2, i3_L_G300_3d_totalRNA_concat_S1_2, i4_L_control_3d_totalRNA_S3,
              i5_L_G300_7d_totalRNA_S4, i7_S_D10_3d_totalRNA_S5, i8_S_G300_3d_totalRNA_S6, i9_S_control_3d_totalRNA_S7)

for(i in 1:length(files)) {
  if(all.equal(rownames(i10_S_G300_7d_totalRNA_S8), rownames(files[[i]]))) {
...
rRNA <- rRNA[,2]
LOUCY_counts <- LOUCY_counts[!LOUCY_counts$geneid %in% rRNA,] # n = 4741209-4741170 = 39 rRNA removed
...
rownames_ERV <- unique(c(rownames_LTR, rownames_ERV)) 
...
input_data_TE <- raw_counts[rownames(raw_counts) %in% rownames_ERV_LINE_SINE, , drop = FALSE] 
stopifnot(nrow(input_data_TE) == length(rownames_ERV_LINE_SINE))
stopifnot(sort(rownames(input_data_TE)) == sort(rownames_ERV_LINE_SINE)) # check that rows are the same
...
counts
write.table(counts$counts, paste0(main_dir, "LOUCY_RNAseq_rawcounts_TE_240806.txt"), sep = "\t", row.names = T)

# CPM and LCPM
cpm <- cpm(counts)
write.table(cpm, paste0(main_dir, "LOUCY_RNAseq_cpm_TE_240806.txt"), sep = "\t", row.names = T)

lcpm <- cpm(counts, log=TRUE)
write.table(lcpm, paste0(main_dir, "LOUCY_RNAseq_lcpm_TE_240806.txt"), sep = "\t", row.names = T)
...
nrow(DEC_Ctrl_test[DEC_Ctrl_test$padjust < 0.05 & DEC_Ctrl_test$logFC > 1, ]) # number of up-regulated TEs

write.table(DEC_Ctrl_test, paste0(main_dir,"LOUCY_DECvsCtrl_RNAseq_exactTest_TE_bcv0.4_240806.txt"), sep = "\t", row.names = T)
...
write.table(GSK_3_Ctrl_test, paste0(main_dir, "LOUCY_GSK3vsCtrl_RNAseq_exactTest_TE_bcv0.4_240806.txt"), sep = "\t", row.names = T)
...
write.table(GSK_7_Ctrl_test, paste0(main_dir, "LOUCY_GSK7vsCtrl_RNAseq_exactTest_TE_bcv0.4_240806.txt"), sep = "\t", row.names = T)
