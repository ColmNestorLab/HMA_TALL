### Pre-processing RNA-seq analysis of count data generated by TEtranscripts

# Title: TElocal_RNAseq_SUPT1_genes_240710
...
set.seed(1040) # seed for reproducibility
...
files <- list(i2_L_D10_3d_totalRNA_S2, i4_L_control_3d_totalRNA_S3,
...
for(i in 1:length(files)) {
  if(all.equal(rownames(i3_L_G300_3d_totalRNA_concat_S1_2), rownames(files[[i]]))) {
...
rRNA <- rRNA[,2]
SUPT1_counts <- SUPT1_counts[!SUPT1_counts$geneid %in% rRNA,] # remove ribosomal RNA n = 4741209-4741170 = 39 rRNA removed from the count matrix 
...
counts
# write.table(counts$counts, paste0("SUPT1_", "RNAseq_rawcounts_genes_240806.txt"), sep = "\t", row.names = T)

# CPM and LCPM
cpm <- cpm(counts)
# write.table(cpm, paste0("SUPT1_","_RNAseq_cpm_genes_240806.txt"), sep = "\t", row.names = T)

lcpm <- cpm(counts, log=TRUE)
# write.table(lcpm, paste0("SUPT1_", "_RNAseq_lcpm_genes_240806.txt"), sep = "\t", row.names = T)
...
bcv = 0.4 # which means dispersion 0.0625. 
...
# write.table(DEC_Ctrl_test, paste0("SUPT1", "_DECvsCtrl_RNAseq_exactTest_genes_bcv0.4_240806.txt"), sep = "\t", row.names = T)
...
# write.table(GSK_3_Ctrl_test, paste0("SUPT1", "_GSK3vsCtrl_RNAseq_exactTest_genes_bcv0.4_240806.txt"), sep = "\t", row.names = T)
...
# write.table(GSK_7_Ctrl_test, paste0("SUPT1", "_GSK7vsCtrl_RNAseq_exactTest_genes_bcv0.4_240806.txt"), sep = "\t", row.names = T)
